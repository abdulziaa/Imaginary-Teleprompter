...
(function() {
    ...
    function loadTextFromURL() {
        ...
        if (textPath && textPath.endsWith('.txt')) {
            // Fetch the text file
            fetch(textPath)
                .then(response => response.text())
                .then(text => {
                    // Convert text to HTML, preserving line breaks but making it one line
                    ...
                    // Store this for initScripts
                    window.initialScriptContent = htmlContent;
                    
                    // If initScripts has already run, update the content directly
                    const scriptsData = sidebar.getElements();
                    if (scriptsData && scriptsData[0]) {
                        scriptsData[0].data = htmlContent;
                        document.getElementById("prompt").innerHTML = htmlContent;
                    }
                })
                .catch(error => {
    ...
    function initScripts() {
        ...
        var sid = sidebar.on('scripts',{
            ...
            "preloadData":[{
                "name": "Instructions",
                "data": window.initialScriptContent || '',
                "editable": false
            }],
        });
